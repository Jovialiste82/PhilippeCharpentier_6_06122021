...
class ImageMediaSubFactory {
  static render(folder, image, title, type) {
    return `<img class="media-${type}-img lb-target" tabindex="0" src="assets/photos/${folder}/${image}" alt="${title}">`;
  }
}

export const mediaFactory = (photographerName, portfolioItem) => {
  const { id, date, image, likes, title, video } = portfolioItem;
  ...
  function getMediaCardDOM() {
    ...
    article.innerHTML = `
      ...
      ${
        image
          ? ImageMediaSubFactory.render(folder, image, title, "card")
...
`;
  ...
  function getMediaSlidesDOM() {
    ...
    article.innerHTML = `
      ...
      ${
        image
          ? ImageMediaSubFactory.render(folder, image, title, "slide")
